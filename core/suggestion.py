...
# Compatibility vocabulary. It is no longer the primary decision mechanism.
...
def _generate_pose_guidance(
    ...
    if not hands_up and elbow_l > 155 and elbow_r > 155:
        guidance.append(Suggestion(
            SuggestionPriority.HIGH, "pose", "把手臂从躯干上分开",
            "任选一只手离开身体几厘米，手肘留一点空间；另一只手可以放在腰侧、腿侧或轻触服装。重点不是“抬手”，而是让轮廓不要粘在一起。",
        ))
    elif hands_up and wrist_y_avg < shoulder_y - 0.12:
        guidance.append(Suggestion(
            SuggestionPriority.MEDIUM, "pose", "上举后先放松手腕",
            "手臂已经打开，不需要继续抬高；让手腕、手指自然弯曲，避免手臂形成僵硬直线。",
        ))

    if knee_avg < 125:
        guidance.append(Suggestion(
            SuggestionPriority.HIGH, "pose", "让双腿产生前后层次",
            "不要让两个膝盖、脚踝落在同一条线上。把其中一条腿略微前伸或后收，画面会更容易读出腿部轮廓。",
        ))
    elif stance < 0.08 and knee_avg > 145:
        guidance.append(Suggestion(
            SuggestionPriority.MEDIUM, "pose", "双脚不要完全并齐",
            "让一只脚稍微前后错开，或把脚尖转出一点角度，保持小幅度即可。",
        ))

    if abs(shoulder_y - hip_y) < 0.08:
        guidance.append(Suggestion(
            SuggestionPriority.MEDIUM, "pose", "给躯干加一点弧线",
            "肩和髋现在比较接近同一高度，试着让一侧肩稍高或身体略向一侧弯，不要主动做大幅度扭转。",
        ))

    if orientation.facing in (
        FacingDirection.FRONT, FacingDirection.FRONT_LEFT, FacingDirection.FRONT_RIGHT,
    ):
        guidance.append(Suggestion(
            SuggestionPriority.MEDIUM, "pose", "头部不要完全锁在正中",
            "身体可以保持当前方向，只把头或下巴轻轻偏向一侧；这样既保留正面交流，又能减少证件照式的平面感。",
        ))
    elif orientation.facing in (
        FacingDirection.BACK, FacingDirection.BACK_LEFT, FacingDirection.BACK_RIGHT,
    ):
        guidance.append(Suggestion(
            SuggestionPriority.MEDIUM, "pose", "保留背面，同时增加头部信息",
    ...
    if px < 0.38:
        guidance.append(Suggestion(
            SuggestionPriority.MEDIUM, "pose", "把视觉动作朝画面中央打开",
            "主体偏左，手臂、膝盖或视线可以略向右打开，避免身体把可用的负空间堵住。",
        ))
    elif px > 0.62:
        guidance.append(Suggestion(
            SuggestionPriority.MEDIUM, "pose", "把视觉动作朝画面中央打开",
            "主体偏右，尽量让手臂、膝盖或视线向左打开，为人物留出呼吸空间。",
        ))

    if orientation.tilt == TiltDirection.LEANING_FORWARD:
        guidance.append(Suggestion(
            SuggestionPriority.MEDIUM, "pose", "前倾可以保留，但从髋部发力",
...
def generate_suggestions(
    ...
    photographer_cues = generate_photographer_cues(
        action, orientation, camera, composition
    )
    ...
    for comp_suggestion in composition.suggestions:
        suggestions.append(Suggestion(
            SuggestionPriority.MEDIUM, "composition", "构图优化", comp_suggestion,
        ))
    ...
    if composition.thirds_alignment < 0.4:
        ...
        suggestions.append(Suggestion(
            SuggestionPriority.MEDIUM, "composition", "让动作服务于画面方向",
