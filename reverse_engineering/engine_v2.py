...
def _analyze_composition_extended(image, pose=None, bbox=None) -> CompositionResult:
    ...
    if thirds_conf > 0.4: styles.append({"name": "rule_of_thirds", "confidence": round(thirds_conf, 2)})
    center_dist = math.hypot(sx - 0.5, sy - 0.5)
    if center_dist < 0.12: styles.append({"name": "center", "confidence": round(1.0 - center_dist / 0.12, 2)})
    ...
    if symmetry > 0.7: styles.append({"name": "symmetry", "confidence": round(float(symmetry), 2)})
...
def _generate_camera_actions(result, candidates=None):
    actions=[]; sx,_=result.composition.subject_position; third_distance=min(abs(sx-tx) for tx in (1/3,2/3))
    if third_distance>0.15: actions.append(CameraAction("MOVE_RIGHT" if sx<0.5 else "MOVE_LEFT", ["subject not near thirds verticals",f"x={sx:.2f}"], "adjust horizontal framing", priority=2))
    if result.composition.subject_scale>0.7: actions.append(CameraAction("MOVE_BACKWARD",[f"subject {result.composition.subject_scale:.0%} of frame"],"reduce subject scale",priority=2))
    elif result.composition.subject_scale<0.1: actions.append(CameraAction("MOVE_FORWARD",[f"subject only {result.composition.subject_scale:.0%}"],"increase subject presence",priority=2))
    if candidates and len(candidates)>1:
        alt=candidates[1]; actions.append(CameraAction("CHANGE_FOCAL_LENGTH",[f"alternative {alt.focal_equiv_35mm:.1f}mm at {alt.distance:.2f}m"],"explore another ranked camera solution",priority=1))
...
class ReverseEngineeringEngineV2:
    ...
    def __init__(self,enable_simulation=True,calibration_profile:CalibrationProfile|str|None=None):
        ...
        self._calibration_profile=resolve_profile(calibration_profile) if isinstance(calibration_profile,str) else (calibration_profile or resolve_profile("Generic"))
    @property
    ...
    def analyze(self,image,pose=None,bbox=None,intrinsics_evidence:IntrinsicsEvidence|None=None):
        ...
        if candidates:
            ...
            camera_pose=CameraPoseResult(camera_height=EstimatedValue(1.5,unit="m",confidence=.1,basis=["no pose"]),camera_distance=EstimatedValue(4.0,unit="m",confidence=.1,basis=["no pose"]),camera_pitch=EstimatedValue(0.0,unit="deg",confidence=.1,basis=["no pose"]),camera_yaw=EstimatedValue(0.0,unit="deg",confidence=.1,basis=["no pose"]),camera_roll=EstimatedValue(0.0,unit="deg",confidence=.1,basis=["no pose"])); focal_length=estimate_focal_length(perspective.perspective_strength.value,perspective.perspective_type.value,composition.subject_scale)
