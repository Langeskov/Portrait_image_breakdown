...
class ReverseEngineeringEngineV2:
    ...
    def __init__(self, enable_simulation=True, calibration_profile: CalibrationProfile | str | None = None):
        self._enable_simulation = bool(enable_simulation)
    ...
    def analyze(self, image, pose=None, bbox=None, intrinsics_evidence: IntrinsicsEvidence | None = None):
        ...
        support_plane = None
        anchor = None
        multi_person_layout = None

        if pose is not None:
            kp = _extract_keypoints_pixels(pose)
            anchor = subject_anchor(kp)
        ...
        if scene_evidence.has_three_directions and scene_evidence.confidence >= 0.45:
            ...
            uncertainties.append("weak-scene roll evidence evaluated independently; yaw/pitch remain pose-derived unless Manhattan geometry is available")
        else:
