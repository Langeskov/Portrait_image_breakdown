"""3D reverse-engineering workspace with multi-person scene validation."""
...
from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QSplitter, QLabel, QDoubleSpinBox, QGroupBox,
    QFormLayout, QListWidget, QListWidgetItem, QSizePolicy, QPushButton,
)

from reverse_engineering.scene import SceneModel, SceneCamera, SceneSubject
from reverse_engineering.data_types import ReverseEngineeringResult
from reverse_engineering.projection import project_scene, ProjectionPreviewResult
...
SUBJECT_COLORS = (
    QColor("#16A34A"), QColor("#7C3AED"), QColor("#DB2777"),
    QColor("#0891B2"), QColor("#EA580C"), QColor("#4F46E5"),
)
...
PERSON_LINKS = [
    (0, 1), (0, 2), (1, 3), (2, 4),
    (0, 0),  # harmless placeholder for compatibility with sparse proxies
    (2, 3), (3, 5), (4, 6), (5, 6), (6, 7),
]
POSE_LINKS = [
...
class SceneView(QWidget):
    ...
    def wheelEvent(self, event):
        self._distance_scale = max(
            34.0,
            min(200.0, self._distance_scale * (1.12 if event.angleDelta().y() > 0 else 0.89)),
        )
        self.update()
    ...
    def _project(self, point):
        ...
        x = float(np.dot(q, right))
        y = float(np.dot(q, up))
        z = float(np.dot(q, forward))
    ...
    def _subject_color(self, index: int):
        return SUBJECT_COLORS[index % len(SUBJECT_COLORS)]

    def _draw_subjects(self, painter):
        ...
        for subject in subjects:
            color = self._subject_color(int(subject.person_index))
            pts = subject.proxy_points() + np.array([
                subject.center_x, subject.center_y, subject.center_z,
            ])
            painter.setPen(QPen(color, 3 if subject is self.scene.subject else 2.4))
            for a, b in links:
                if a < len(pts) and b < len(pts):
                    painter.drawLine(self._project(pts[a]), self._project(pts[b]))
            painter.setBrush(QBrush(color))
            for p in pts:
                painter.drawEllipse(self._project(p), 4 if subject is self.scene.subject else 3.2, 4 if subject is self.scene.subject else 3.2)

            anchor = np.mean(pts[[6, 7]], axis=0) if len(pts) >= 8 else pts.mean(axis=0)
            label = f"P{int(subject.person_index) + 1}"
            if subject.depth_is_relative:
                label += f"  z={subject.center_z:+.2f}  ({subject.depth_confidence:.0%})"
            else:
                label += "  2D-only"
            lp = self._project(anchor)
            painter.setPen(QPen(color, 1))
            painter.setFont(QFont("Segoe UI", 8, QFont.Weight.Bold))
            painter.drawText(lp + QPointF(7, -7), label)
    ...
    def _draw_camera_body(self, painter, position):
        ...
        back = pos - forward * body_len * 0.5
        front = pos + forward * body_len * 0.5
        corners = [
            back + right * body_w + up * body_h,
            back - right * body_w + up * body_h,
            back - right * body_w - up * body_h,
            back + right * body_w - up * body_h,
            front + right * body_w + up * body_h,
            front - right * body_w + up * body_h,
            front - right * body_w - up * body_h,
            front + right * body_w - up * body_h,
        ]
    ...
    def _draw_candidate_cameras(self, painter):
        ...
        for i, candidate in enumerate(self.scene.candidate_solutions):
            if i == self.scene.selected_candidate:
                continue
            yaw = math.radians(float(getattr(candidate.extrinsics, "yaw", 0.0)))
            p = np.array([
                self.scene.subject.center_x + math.sin(yaw) * candidate.distance,
                candidate.height,
                self.scene.subject.center_z - math.cos(yaw) * candidate.distance,
            ])
    ...
    def _draw_camera(self, painter):
        ...
        subject_target = self.scene.camera_target()
        optical_target = self.scene.camera_aim_target()
        forward, right, up = self._camera_axes()
        ...
        frame = [
            far + right * hw + up * hh, far - right * hw + up * hh,
            far - right * hw - up * hh, far + right * hw - up * hh,
        ]
        self._line(painter, position, position + forward * near, CAMERA, 3)
        self._line(painter, position + forward * near, far, CAMERA, 2, True)
        self._line(painter, position, optical_target, CAMERA, 2)
        self._line(painter, optical_target, subject_target, OBSERVED, 1, True)
        ...
        tp = self._project(optical_target)
        painter.setPen(QPen(CAMERA, 2))
        ...
        sp = self._project(subject_target)
        painter.setPen(QPen(OBSERVED, 1))
    ...
    def _draw_labels(self, painter):
        ...
        c = self.scene.camera
        aim_error = self.scene.camera_aim_error()
        count = len(self.scene.subjects) if self.scene.subjects else 1
        rel = sum(1 for s in self.scene.subjects if s.depth_is_relative) if self.scene.subjects else 0
        painter.drawText(12, 22, "3D scene — drag to orbit, wheel to zoom")
        painter.drawText(12, 40, "Camera / frustum / multi-person proxy / optical axis")
        painter.drawText(12, 58, f"FOV {c.horizontal_fov_deg:.1f}° × {c.vertical_fov_deg:.1f}°   YAW {c.yaw:+.1f}°   PITCH {c.pitch:+.1f}°   ROLL {c.roll:+.1f}°")
        painter.drawText(12, 76, f"Optical-axis miss: {aim_error:.2f} m   People: {count}   Relative-3D: {rel}/{count}")
...
class ProjectionPreview(QWidget):
    def __init__(self, parent=None):
        ...
        self._projections: tuple[ProjectionPreviewResult, ...] = ()
    ...
    def update_projection(self, scene, observed_bbox=None, observed_points=None):
        ...
        self._projections = project_scene(scene, w, h)
        primary = next((p for p in self._projections if p.person_index == scene.subject.person_index), self._projections[0] if self._projections else None)
        if primary is None or primary.bbox is None:
            self._metrics = "3D scene is outside the camera-facing half-space"
            self.update()
            return

        x0, y0, x1, y1 = primary.bbox
        metrics = [
            f"3D primary {max(0, x1-x0)/max(w,1):.0%} W × {max(0, y1-y0)/max(h,1):.0%} H",
            f"{len(self._projections)} projected people",
        ]
    ...
    def paintEvent(self, event):
        ...
        if self._pixmap is None:
            ...
            painter.drawText(self.rect(), Qt.AlignCenter, "Projection preview")
            painter.end()
            return
        ...
        def rect_of(r):
            ...
            return QRectF(ox + x0*sx, oy + y0*sy, (x1-x0)*sx, (y1-y0)*sy)

        def pt(x, y):
            return self._map(x, y, ox, oy, sx, sy)

        for idx, points in enumerate(self._observed_points):
            points = np.asarray(points, dtype=float)
            col = OBSERVED if idx == 0 else SUBJECT_COLORS[idx % len(SUBJECT_COLORS)]
            painter.setPen(QPen(col, 1.6))
            for a, b in POSE_LINKS:
                if a < len(points) and b < len(points) and np.isfinite(points[[a,b]]).all():
                    painter.drawLine(pt(points[a,0], points[a,1]), pt(points[b,0], points[b,1]))
            painter.setBrush(QBrush(col))
            for p in points:
                if np.isfinite(p[:2]).all():
                    painter.drawEllipse(pt(p[0], p[1]), 2.6, 2.6)

        if self._observed_bbox is not None and len(self._observed_bbox) >= 4:
            r = rect_of(self._observed_bbox)
            painter.setPen(QPen(OBSERVED, 2, Qt.DashLine))
            painter.setBrush(Qt.NoBrush)
            painter.drawRect(r)
            painter.setFont(QFont("Segoe UI", 8))
            painter.drawText(r.topLeft() + QPointF(3, -4), "OBSERVED PRIMARY")

        for proj in self._projections:
            if proj.bbox is None:
                continue
            col = SUBJECT_COLORS[proj.person_index % len(SUBJECT_COLORS)]
            r = rect_of(proj.bbox)
            painter.setPen(QPen(col, 2 if proj.person_index == 0 else 1.5))
            painter.setBrush(Qt.NoBrush)
            painter.drawRect(r)
            painter.setFont(QFont("Segoe UI", 8, QFont.Weight.Bold))
            painter.drawText(r.topRight() + QPointF(-26, -4), f"P{proj.person_index + 1}")
            center = r.center()
        ...
        painter.setFont(QFont("Segoe UI", 8))
        painter.drawText(8, self.height()-10, self._metrics)
        painter.end()
...
class Reverse3DWorkspace(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._result: Optional[ReverseEngineeringResult] = None
        ...
        llo.setContentsMargins(0, 0, 0, 0)
        self._view = SceneView(self.scene)
        llo.addWidget(self._view, 1)
        reset = QPushButton("Reset 3D view")
        reset.clicked.connect(self._view.reset_view)
        reset.setMaximumWidth(130)
        llo.addWidget(reset, 0, Qt.AlignRight)
        splitter.addWidget(left)
        splitter.addWidget(self._build_panel())
        splitter.setSizes([940, 470])
    ...
    def _build_panel(self):
        ...
        form = QFormLayout(camera_group)
        self._distance = self._spin(.1, 50, .1, " m")
        self._height = self._spin(.1, 5, .05, " m")
        ...
        for box in (self._distance, self._height, self._yaw, self._pitch, self._roll, self._focal):
            ...
            box.setToolTip("可使用鼠标滚轮微调参数，实时观察 2D / 3D 复现效果。")
        ...
        lo.addWidget(camera_group)

        people_group = QGroupBox("Scene people")
        people_layout = QVBoxLayout(people_group)
        self._people = QListWidget()
        self._people.setMinimumHeight(105)
        people_layout.addWidget(self._people)
        people_hint = QLabel("绿色/紫色/… = independent person proxy; z is normalized relative depth, not meters")
        people_hint.setWordWrap(True)
        people_hint.setStyleSheet("color:#64748B; font-size:9pt;")
        people_layout.addWidget(people_hint)
        lo.addWidget(people_group)

        pg = QGroupBox("2D projection preview")
        plo = QVBoxLayout(pg)
        self._preview = ProjectionPreview()
        plo.addWidget(self._preview)
        ...
        plo.addWidget(self._preview_metrics)
        note = QLabel("Orange = primary observed pose · other colors = additional people / 3D projected proxies")
        note.setStyleSheet("color:#64748B; font-size:9pt;")
        note.setWordWrap(True)
        plo.addWidget(note)
        lo.addWidget(pg)

        cg = QGroupBox("Candidate solutions")
        clo = QVBoxLayout(cg)
        self._candidates = QListWidget()
        self._candidates.currentRowChanged.connect(self._select_candidate)
        clo.addWidget(self._candidates)
        lo.addWidget(cg, 1)

        self._note = QLabel("Single-image reconstruction remains multi-solution. Primary camera fitting is preserved; additional people are rendered from conservative image-space and optional relative-depth layout evidence.")
    ...
    def set_result(self, result, observed_bbox=None, observed_points=None):
        ...
        if result:
            ...
            self._note.setText(
                "3D people are rendered from the shared camera and scene layout. "
                "Relative z is normalized only; metric room coordinates remain unknown."
                if rel3d else
                "Multiple people are retained in image-space and rendered with independent proxies. "
                "No metric inter-person distance is inferred without independent depth evidence."
            )
        else:
    ...
    def _populate_people(self):
        self._people.clear()
        subjects = list(self.scene.subjects) if self.scene.subjects else [self.scene.subject]
        layout = getattr(self._result, "multi_person_layout", None) if self._result else None
        for subject in subjects:
            p = None
            if layout is not None:
                p = next((x for x in layout.people if int(x.person_index) == int(subject.person_index)), None)
            color_name = ["green", "purple", "magenta", "cyan", "orange", "indigo"][subject.person_index % 6]
            depth_text = f"z={subject.center_z:+.2f}" if subject.depth_is_relative else "2D-only"
            conf_text = f" · depth {subject.depth_confidence:.0%}" if subject.depth_is_relative else ""
            role = "primary" if subject is self.scene.subject else "additional"
            self._people.addItem(QListWidgetItem(
                f"P{subject.person_index+1} · {role} · {color_name} · {depth_text}{conf_text}"
            ))
    ...
    def update_results(self, bundle):
        if getattr(bundle, "reverse_result", None):
            ...
            if pose is not None:
                ...
                self._observed_points = [
                    np.array([[lm.x, lm.y] for lm in p.landmarks[:17]], dtype=float)
                    for p in people
                ]
