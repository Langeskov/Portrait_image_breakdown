...
from PySide6.QtWidgets import (
    ...
    QComboBox, QVBoxLayout, QWidget, QApplication,
)
...
class FieldModeWidget(QWidget):
    """High-signal field UI with a stable visual hierarchy."""
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("""
            ...
            QFrame#statusCard { background: #18202b; border: 1px solid #2c3847; border-radius: 8px; }
            QLabel#eyebrow { color: #8fa3b8; font-size: 11px; letter-spacing: 1px; }
            QLabel#primaryCue { color: #ffffff; padding: 8px; }
            QLabel#status { color: #aebdcd; }
            QLabel#metric { color: #cbd5e1; padding: 5px 8px; background: #18202b; border-radius: 5px; }
            QListWidget { background: #141b24; border: 1px solid #293542; border-radius: 6px; padding: 4px; }
            QListWidget::item { padding: 9px 8px; }
            QPushButton { padding: 7px 12px; border: 1px solid #3a4858; border-radius: 5px; background: #1b2530; color: #e5e7eb; }
            QPushButton:disabled { color: #657384; background: #161d25; }
            QComboBox { padding: 6px 10px; border: 1px solid #3a4858; border-radius: 5px; background: #151e28; color: #e5e7eb; }
        """)
        ...
        secondary_header.addWidget(QLabel("SECONDARY CUES"))
    ...
    def _copy_ssml(self):
        snap = self.history.current
        text = ssml(snap.summary if snap else "")
        QApplication.clipboard().setText(text)
