...
Keeps the current cue, a short history, and technical confidence visible at
once. No speech engine is required; the cue text is ready for any TTS layer.
"""
...
class FieldModeWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        row = QHBoxLayout(); row.addWidget(QLabel("Cue mode")); self._mode = QComboBox(); self._mode.addItems([m.value for m in CueMode]); row.addWidget(self._mode); row.addStretch(1); lo.addLayout(row)
        ...
        self._undo.clicked.connect(self._on_undo); self._redo.clicked.connect(self._on_redo); self._copy.clicked.connect(self._copy_voice)
        self._refresh_buttons()

    def set_cues(self, cues, confidence: float | None = None):
        self._cues = list(cues or [])
        snap = self.history.push(self._cues)
        self._status.setText(f"Analysis confidence: {confidence:.0%}" if confidence is not None else "Analysis ready")
        self._render_current()

    def _mode_enum(self) -> CueMode:
        value = self._mode.currentText()
        return next((m for m in CueMode if m.value == value), CueMode.NORMAL)

    def _render_current(self):
        snap = self.history.current
        if snap is None:
            return
        lines = list(snap.cue_text)
        if self._cues:
            formatted = format_cues(self._cues, self._mode_enum())
            self._detail.clear()
            for line in formatted[:6]: self._detail.addItem(QListWidgetItem(line))
        else:
            self._detail.clear()
        self._primary.setText(snap.summary)
        self._refresh_buttons()
        self.cue_changed.emit(voice_ready_text(snap.summary))

    def _on_undo(self):
        snap = self.history.undo()
        if snap: self._primary.setText(snap.summary)
        self._refresh_buttons()
        self.cue_changed.emit(voice_ready_text(snap.summary if snap else ""))
    ...
    def _refresh_buttons(self):
        self._undo.setEnabled(self.history.can_undo); self._redo.setEnabled(self.history.can_redo)

    def _copy_voice(self):
        snap = self.history.current
        if snap:
            QApplication.clipboard().setText(voice_ready_text(snap.summary))

    def voice_payload(self) -> dict[str, str]:
        snap = self.history.current
