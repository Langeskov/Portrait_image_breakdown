...
class PlaneConstraintPanel(QWidget):
    ...
    def _apply(self):
        ...
        position = (subject.center_x, subject.center_y, subject.center_z)
        try:
            x, y, z = apply_position_constraint(position, constraint, self.workspace.scene.anchors)
    ...
    def _evaluate(self):
        ...
        subject = self.workspace.scene.subject
        try:
            result = evaluate_constraint((subject.center_x, subject.center_y, subject.center_z), constraint, self.workspace.scene.anchors)
...
class TemporalWorker(QThread):
    progressed = Signal(int, float, float, int)
    completed = Signal()
    failed = Signal(str)
        ...
        try:
            ...
            while not self.stop_requested:
                ...
                if index % stride == 0:
                    ...
                    visible = int(np.sum(state.keypoints[:, 0] >= 0)) if state.keypoints is not None else 0
                    self.progressed.emit(index, state.confidence, float(np.nanmean(state.keypoints)) if state.keypoints is not None else 0.0, visible)
                index += 1
                last_frame = index
            capture.release()
            self.completed.emit()
...
class TemporalWorkspace(QWidget):
    def __init__(self, window, parent=None):
        ...
        root.addWidget(title)
        hint = QLabel("视频序列模式：对连续帧的人体关键点进行时间平滑。相机状态通过 TemporalSmoother API 保留，当前 UI 不会假装从视频单帧恢复绝对相机。")
        ...
        self.alpha = QDoubleSpinBox(); self.alpha.setRange(0.05, 1.0); self.alpha.setSingleStep(0.05); self.alpha.setValue(0.35)
        bar.addWidget(QLabel("Smoothing")); bar.addWidget(self.alpha)
        ...
        root.addWidget(self.frame); root.addWidget(self.confidence); root.addWidget(self.visible)
        root.addStretch(1)
    ...
    def _progress(self, index, confidence, _, visible):
...
def install_v3_completion(window):
    """Install the remaining v3 features without replacing existing v2/v2.5 paths."""
    toolbar = window.findChildren(type(window.findChildren(QWidget)[0])) if window.findChildren(QWidget) else []
    ...
    def save_action():
        ...
        if not path:
            return
        constraints = getattr(getattr(window, "_w3", None), "_plane_constraints_panel", None)
        data = constraints.to_dicts() if constraints is not None else getattr(window._w3.scene, "plane_constraints", [])
        save_session(path, window._w3.scene, image_path=getattr(window, "_current_path", None), image_shape=window._img.shape[:2][::-1] if window._img is not None else None, plane_constraints=data)
    ...
    if bar is not None:
        bar.addSeparator()
        save = QAction("Save Session", window); save.triggered.connect(save_action); bar.addAction(save)
        load = QAction("Load Session", window); load.triggered.connect(load_action); bar.addAction(load)

    # Plane constraints are inserted immediately after Scene anchors, preserving the existing ordering.
    if hasattr(window, "_w3"):
        ...
        if scrolls:
            ...
            if anchors is not None:
                layout.insertWidget(layout.indexOf(anchors) + 1, panel)
            else:
                layout.insertWidget(2, panel)
        window._w3._plane_constraints_panel = panel

    # Composition-aware target generation on the Reference workspace.
    ref = getattr(window, "_reference_mode", None)
    if ref is not None:
        target_box = QGroupBox("Generated composition-aware pose target")
        target_layout = QVBoxLayout(target_box)
        target_label = QLabel("Ready when a reference and current pose are available.")
        target_label.setWordWrap(True)
        target_layout.addWidget(target_label)
        generate = QPushButton("Generate target")
        target_layout.addWidget(generate)
        def generate_target():
            if ref._reference_pose is None or ref._reference is None or ref._current_pose is None:
                target_label.setText("Reference + current pose are required.")
                return
            width, height = ref._current_image_size
            current = __import__("reverse_engineering.reference_reconstruction", fromlist=["build_reference_composition"]).build_reference_composition(ref._current_pose, width, height)
            result = generate_composition_aware_pose_target(ref._reference_pose.landmarks, ref._reference, current)
            target_label.setText(result.message if not result.success else f"{result.message} · target center {result.target.subject_center[0]/width:.0%}, {result.target.subject_center[1]/height:.0%}")
        generate.clicked.connect(generate_target)
        ref.layout().addWidget(target_box)
