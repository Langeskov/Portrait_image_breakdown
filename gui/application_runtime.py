...
def _save_reconstruction_session(window) -> None:
    ...
    try:
        ...
        window._st.showMessage(f"Saved reconstruction session · {path.rsplit('/', 1)[-1]}")
...
def _install_temporal_session_controls(window) -> None:
    ...
    hint = QLabel(
        "Save / Load preserves the editable V3 reconstruction scene, anchors, "
        "plane constraints and session metadata."
    )
    hint.setWordWrap(True)
    hint.setStyleSheet("color:#64748B;")
    layout.addWidget(hint)
